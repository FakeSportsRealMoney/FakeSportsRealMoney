...
cronJobs.overdueCheck = new CronJob('00 30 15 * * *', function() {
  request(localUrl, (err, response, html) => {
    let leagues = JSON.parse(response.body);
    leagues.forEach((league) => {
      request(localUrl + league._id + '/user/overdue', (err, response, html) => {
  ...
  request(localUrl, (err, response, html) => {
    ...
    leagues.forEach((league) => {
      request(localUrl + league._id + '/user/overdue', (err, response, html) => {
        let overdueMembers = JSON.parse(response.body);
        overdueMembers.forEach((member) => {
          request(localUrl + member.leagueId + '/user/' + member._id + '/increment', (err, response, html) => {
